<section class="{{$field->getClassName()}}">
    <div class="tab-pane active">
        ...
        <ul class="nav nav-tabs tabs-pull-right">
            ...
            @foreach ($field->getLanguage() as $tab)
                <li class="{{$loop->first ? 'active' : ''}}">
                    <a href="#{{$definition->getNameDefinition() . $field->getNameField() . $tab['caption']}}" data-toggle="tab">{{__cms($tab['caption'])}}</a>
                </li>
            @endforeach
        </ul>
        ...
        <div class="tab-content padding-5">
            @foreach ($field->getLanguage() as $tab)
                <div class="tab-pane {{ $loop->first ? 'active' : '' }}" id="{{$definition->getNameDefinition() . $field->getNameField() . $tab['caption']}}">
